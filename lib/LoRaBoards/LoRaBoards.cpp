...
// ---------- u-blox helpers (UBX) ----------
static bool mem_contains(const uint8_t *buf, int len, const char *needle)
{
    ...
    for (int i = 0; i <= len - nlen; ++i) {
        int j = 0;
        for (; j < nlen; ++j) {
            if ((char)buf[i + j] != needle[j]) break;
        }
...
// Lê um frame UBX e retorna o payload.
// Retorna payload_len (>0) se leu; 0 se timeout/erro.
static int ubx_read_frame(uint8_t *payload, uint16_t payload_max,
                          uint8_t &cls, uint8_t &id,
                          uint32_t timeout_ms = 800)
{
    enum { S_SYNC1, S_SYNC2, S_CLASS, S_ID, S_LEN1, S_LEN2, S_PAYLOAD, S_CK_A, S_CK_B } st = S_SYNC1;
    ...
    while (millis() - start < timeout_ms) {
        while (SerialGPS.available()) {
            uint8_t c = (uint8_t)SerialGPS.read();
            ...
            case S_LEN2:
                ...
                if (len == 0) {
                    st = S_CK_A;
                } else if (len > payload_max) {
                    // descarta payload grande demais
                    for (uint16_t k = 0; k < len; ++k) {
                        if (!SerialGPS.available()) break;
                        (void)SerialGPS.read();
                    }
                    st = S_CK_A;
                } else {
            ...
            case S_PAYLOAD:
                ...
                break;
            case S_CK_A:
                st = S_CK_B; // ignora checksum (poderia validar)
                break;
            case S_CK_B:
                return (int)len;
            }
...
static bool ubx_wait_ack(uint8_t reqClass, uint8_t reqId, uint32_t timeout_ms = 1200)
{
    uint32_t start = millis();
    uint8_t cls = 0, id = 0;
    uint8_t p[16];
    ...
    while (millis() - start < timeout_ms) {
        int n = ubx_read_frame(p, sizeof(p), cls, id, timeout_ms);
        if (n < 2) continue;

        // ACK-ACK
        if (cls == 0x05 && id == 0x01 && p[0] == reqClass && p[1] == reqId) return true;
        // ACK-NAK
        if (cls == 0x05 && id == 0x00 && p[0] == reqClass && p[1] == reqId) return false;
    }
    return false;
}

// Detecta u-blox pedindo MON-VER (responde com strings contendo "u-blox" na maioria dos firmwares)
static bool ubloxProbe()
{
    // UBX-MON-VER poll: B5 62 0A 04 00 00 0E 34
    const uint8_t mon_ver[] = {0xB5,0x62,0x0A,0x04,0x00,0x00,0x0E,0x34};

    while (SerialGPS.available()) SerialGPS.read();  // limpa lixo/NMEA
    SerialGPS.write(mon_ver, sizeof(mon_ver));

    uint8_t cls = 0, id = 0;
    uint8_t payload[240];
    int n = ubx_read_frame(payload, sizeof(payload), cls, id, 800);

    if (n > 0 && cls == 0x0A && id == 0x04) {
        // payload tem strings fixas; busca "u-blox" no buffer bruto
        if (mem_contains(payload, n, "u-blox")) return true;
        // fallback: só o fato de responder MON-VER já é um bom sinal de UBX vivo
        return true;
    }

    // Fallback NMEA proprietário u-blox (se preferir): $PUBX,00*33
    // (Você pode descomentar se quiser testar como 2ª tentativa.)
    /*
    while (SerialGPS.available()) SerialGPS.read();
    SerialGPS.print("$PUBX,00*33\r\n");
    uint32_t t0 = millis();
    while (millis() - t0 < 300) {
        if (SerialGPS.find((char*)"$PUBX,00")) return true;
    }
    */
    return false;
}

// Configura taxa de navegação (ex.: 5 Hz) via UBX-CFG-RATE
static bool ubloxSetRate5Hz()
{
    // UBX-CFG-RATE: measRate=200ms (0x00C8), navRate=1, timeRef=1
    // Msg: B5 62 06 08 06 00 C8 00 01 00 01 00 DE 6A
    const uint8_t cfg_rate_5hz[] = {0xB5,0x62,0x06,0x08,0x06,0x00,0xC8,0x00,0x01,0x00,0x01,0x00,0xDE,0x6A};

    while (SerialGPS.available()) SerialGPS.read();
    SerialGPS.write(cfg_rate_5hz, sizeof(cfg_rate_5hz));
    return ubx_wait_ack(0x06, 0x08, 1200);
}
